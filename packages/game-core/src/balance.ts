...
export type ThermalLevel = keyof typeof THERMAL_MULTIPLIERS;

export function spawnRate(elapsedSec: number, playerCount: number): number {
 ...
 * the cumulative loop.
 */
export function levelFromXp(totalXp: number): { level: number; intoLevel: number; need: number } {
  let level = 1;
